...
`open_topic_conversation_v1`은 배너에 보인 질문으로 대화를 준비한다. [주제 대화](BANNER_TOPICS_DESIGN.md)에 선택·첫 답변·복구·콘텐츠 운영 규칙을 둔다. `open_conversation`은 일반 대화 진입이며 답변하지 않고 나간 주제 질문을 복원하지 않는다. 캐피의 일기 목록·감정 기록·루틴·타이머 설정·음악 선택 이동도 지원하며 action 이름은 공동 규약을 따른다. `open_affirmation_screen_v1`은 오늘의 문장 전체화면을 연다. 배너에서는 확인 상태를 저장하지 않고 전체화면의 확인 버튼에서 저장한다. 같은 이름의 capability를 지원하는 클라이언트 빌드가 필요하다. 새 action을 지원하지 않는 앱에는 해당 카드를 제공하지 않는다.
...
버튼의 실제 터치 영역은 최소 48px로 확장된다. 본문 frame이 이 영역과 겹치면 앱이 카드 전체를 숨긴다. 문장 위치와 클릭 영역을 함께 검증한다. 운세·주제·상점·감정 배너의 본문은 짧은 선의 아래 경계와 버튼 배경의 위 경계 사이에 중앙 정렬한다. 운세는 날짜·질문 두 줄 전체를 하나의 묶음으로 중앙 정렬한다. 버튼 그림과 최소 클릭 영역은 서로 다를 수 있다.
...
이미지 버튼의 응답 예시는 [composed_feed.json](../tests/fixtures/banners/composed_feed.json)을 참고한다. 현재 배너 파일은 운세·오늘의 문장·기능 업데이트(감정 기록)·상점·대화 주제 순서로 배경, 제목, 본문과 버튼을 정의한다. 운세 날짜는 `date` 요소의 `{day}`로 요청 시간대의 오늘을 표시하고, 질문은 `message` 요소로 분리해 굵기를 각각 조절한다. 오늘의 문장 카드는 안내 문구와 문장 보기 버튼만 표시한다. 실제 문장은 전체화면에서 조회하고 해당 화면의 확인 버튼으로 당일 확인을 저장한다. 나머지 카드는 `shape_v1` + `text_v1` + `action_region_v1` 버튼이다. 버튼은 각각 기존 운세 화면, 오늘의 문장 전체화면, 감정 기록 팝업, 상점 화면, 주제 준비를 거친 대화 화면으로 이동한다. 음악 추천 카드는 현재 제공하지 않는다. 정확한 필수 필드는 [서버 스키마](../app/schemas/banners.py)를 따른다.
...
| `affirmation.acknowledged_today` | 요청 사용자가 오늘 현지 날짜에 오늘의 문장를 확인했으면 1, 아니면 0. format은 null. `when`은 `eq 0` 필수이므로 확인한 날에는 카드가 빠진다. 확인은 `POST /daily-affirmation/acknowledge`, 문장은 `GET /daily-affirmation`이 담당하며 문장 자체는 배너 문구에 넣지 않는다 |
...
오늘의 문장 문장의 편집 원본은 [app/resources/affirmations.json](../app/resources/affirmations.json)이며 ko·en·ja를 함께 적는다. 문장은 현지 날짜 서수를 문장 수로 나눈 주기마다 고정 순열을 돌려 고르므로(한 주기 안에서 모든 문장이 한 번씩, 이틀 연속 같은 문장 없음) 같은 날짜에는 전원이 같은 문장을 받고 DB·배치·LLM을 쓰지 않는다.
...
문구는 `{"kind":"template","value":"오늘은 {day}"}`처럼 작성한다. `{day}`는 등록한 binding alias이며 literal 중괄호는 `{{`/`}}`로 쓴다. `count_cases`는 0/1/그 외 문구를 선택한다. `when`은 binding의 `eq` 또는 `gt` 조건이며 루틴 배너에는 `remaining > 0`, 오늘의 문장 배너에는 `acknowledged == 0` 조건을 둔다.
...
## 2026-09-23 개발 배너 노출 변경

사용자 요청으로 개발 카탈로그 `affirmation-daily`의 enabled를 false로 설정했다. 오늘의 문장 배너는 모든 앱 언어·플랫폼에서 노출하지 않는다. 문장 조회·확인 API 및 배너 정의는 유지하며, 이 변경은 dev 브랜치에만 배포한다.
