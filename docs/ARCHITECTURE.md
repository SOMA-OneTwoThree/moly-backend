...
| 09:00 | 아침 일기 도착 푸시. **현재 꺼져 있다**(`morning_push_enabled` 기본값 False). 새로 공개된 전날 미독 개인/운영자 일기만 대상으로 한다. 대상 판정·클라 이동·환경별 활성화는 OPERATIONS의 아침 일기 푸시 절을 따른다 |
...
운영자 일기는 `app_config.diary_weekly_start_date`부터 활동일이 속한 주의 미수령 원고를
순서대로 지급한다. 개인 일기가 우선이며 운영 원고가 없거나 소진됐으면 미발행 결과만 남긴다.
사용자별 최종 트랜잭션 잠금 안에서 후보를 재확인하고 일기·주간 지급 기록·회상 잡을 함께
커밋한다. 원고는 FOR SHARE로 확인하여 다른 사용자들의 같은 원고 지급은 병렬로 처리한다.
번역·개인 LLM 호출은 DB 읽기 트랜잭션을 닫고 수행한다. 지급 이력은 일기 삭제와 별개로 유지한다.
