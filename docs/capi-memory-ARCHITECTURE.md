...
> 상태: **Dev 구현 진행 중 — cutover 전** (2026-08-06 갱신)
> 작성일: 2026-08-05
>
> 구현된 것: 0~9단계 · v2 읽기 경로 · provenance · 계약/관계 live 주입 · 다국어 · 큐 분리.
> 남은 것: golden 200건(제품 판단) · soak/장애주입 · 두 번 연속 gate · 그 뒤 cohort cutover.
> **자동 게이트는 현재 정직하게 실패한다** — 존재 검사(provenance·계약·관계 render)가 붙어 있고,
> 데이터가 없으면 통과가 아니라 실패다. `scripts/verify_cutover_gate.py`가 판정한다.
