...
- 생년월일 또는 성별이 실제로 바뀌면 revision을 올린다. 이미 발급한 당일 결과는 유지하고 다음 날짜의 새 결과부터 변경을 반영한다(`result_invalidated=false`).
...
새 정책의 규칙 버전은 `fortune-experience.v1`이다. 개발 환경에서는 새 정책을 사용하며, 운영에서는 `app_config.fortune_experience_start_date` 전환일부터 적용한다. 운영 키가 없거나 잘못되면 기존 `fortune-independent.v2-floor30` 점수와 UUID 기반 카드 선택을 유지한다. 새 원고는 새 이미지에서 생성되는 결과에 사용한다.
...
이미 발급한 당일 결과는 문구 버전 변경이나 프로필 수정으로 교체하지 않는다. 날짜가 바뀌면 해당 날짜의 결과를 생성한다. 프로필 삭제는 명시적인 결과 삭제이며, 재등록 시 같은 생일·날짜·모드·버전에서 재현한다. 최신 한 건 저장 구조이므로 과거 날짜의 옛 문구 버전까지 영구 복원하지 않는다.
...
사용자당 한 행만 두고 날짜가 바뀌면 같은 행을 새 snapshot으로 교체한다. 당일 프로필 수정은 저장된 결과를 교체하지 않는다. 무기한 일별 이력을
쌓지 않는다.
...
운영의 새 점수·사람 간 동일 추첨 전환은 `app_config.fortune_experience_start_date`로 제어한다. 전환일을 넣기 전에 모든 인스턴스가 새 저장 형식을 읽을 수 있는 이미지인지 확인한다. 값이 없거나 잘못되면 기존 계산·추첨 경로를 유지한다. 설정 방식과 전환·복구 순서는 [긍정 경험 배포 계획](fortune-content/positive-experience-plan.md)을 따른다. 이번 작업에서 전환일 설정, 원격 push, PR 생성 또는 신규 이미지 배포를 수행하지 않는다.
