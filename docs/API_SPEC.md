...
- 이 기간엔 구독 없이 **전원 무료** — 등급 `trial`, 일 토큰 한도 = **런칭 한도 150,000**. 배너 광고는 끄지 않는다(`ads_removed=false`, 배너는 새 앱에만 있다). `/chat/state`·`/subscription` 모두 `in_trial:true`, `trial_ends_at`=런칭 종료. 실제 구독자는 항상 우선.
- 종료일 `app_config.free_launch_until`·한도 `free_launch_token_limit` → **재배포 없이 조정**. 종료 시 자동으로 정상 등급 복귀.

`free_launch_until`이 스토어 출시 시각 T다. T 전에는 구독·앱 체험이 없는 계정이 런칭 무료를 유지하고, T부터 전환 정책을 적용한다. 저장된 계정 언어 기준으로 무료는 영어 40,000·한국어 50,000·일본어 60,000, 구독·가입 체험·스토어 체험은 영어 300,000·한국어 400,000·일본어 550,000이며(구독·체험 한도는 T 전에도 같다), 전환이나 결제 시 당일 사용량을 초기화하지 않는다. T 이전 가입자는 기존 회원(스토어 1개월 체험 오퍼), T 이후 가입자는 신규 회원(최초 가입 시각 +48시간 체험)이다. T 전에는 `subscription_launch.new_member_since` 이후 가입한 계정을 신규 회원으로 보아, 새 앱의 테스터·심사자가 두 페이월을 모두 볼 수 있다. 기존에 발급한 체험과 이미 받은 일기는 유지한다.
...
> 가입 직후부터 대화·닉네임 입력 여부와 무관하게 `kind:"welcome"` 첫 만남 일기를 받을 수 있다.
> 첫 `GET /diaries`에서 가입 시각과 조회 당시 프로필 시간대를 기준으로 생성·발행하며, 이후 날짜는 고정된다.
> 기존 사용자에게 누락된 일기도 같은 조회에서 생성한다. welcome은 일일 슬롯과 별개이며 daily는 다음날
...
- 첫 만남 일기는 대화 여부와 무관하게 가입 후 첫 목록 조회에서 한 번만 생성한다. 기존 일기가 있으면 다시 만들지 않는다.
...
- `price_hay:null`은 비매품이다. v2 상품의 `subscriber_only:true`는 활성 구독 중에만 구매 없이 장착하는 상품이며 가격은 항상 null이다. 구독이 끝나면 장착·카탈로그 조회가 테마는 `theme_default`, 착용 아이템은 해제로 응답한다. 실제 가격과 노출 여부는 DB 상품 카탈로그가 기준이다.
...
| 상점 | 가격·노출 여부는 DB의 활성 상품 카탈로그가 기준. 구독 전용 꾸미기는 활성 구독 중에만 장착 |
