...
**신규 가입자는 첫 대화에서 등록된다.** `chat._record_memory_v2`가 상태 행이 없는 사용자를 보면
`memory_pipeline.enroll`로 `v2`·`ready`·커서 0인 행을 만든다. 과거 대화가 없으니 채울 범위도
없어 바로 `ready`로 둬도 된다. 이 경로가 없던 동안에는 새로 가입한 사람마다 **평생 기억이
0건**이었다 — 상태 행을 만드는 코드가 이관용 스크립트에만 있었고, 행이 없으면 `legacy`로
해석돼 아무 일도 하지 않았다. 오류도 로그도 남지 않았다(2026-08-08 점검에서 발견).
행이 이미 있는데 `legacy`면 일부러 꺼둔 것으로 보고 건드리지 않는다.

### 6.2 저장 과정

담당 코드는 `worker/mem0_jobs.py`의 `handle_mem0_ingest`와 `app/services/mem0_pipeline.py`다.

작업의 멱등 키는 **이 작업이 출발할 커서**로 만든다(`mem0:{user}:c{커서}:v1[:g{세대}]`).
처리 대상 턴으로 만들면 안 된다 — 실제 처리는 대화 덩어리가 끊기는 데까지만 하므로, 덩어리가
요청한 턴보다 앞에서 끊기면(하루 경계 등) 다음 작업의 키가 **자기 자신의 키와 같아진다.**
`ON CONFLICT DO NOTHING`이라 작업은 안 생기고 오류도 안 난다 — 그 사용자의 기억이 조용히
영원히 멈춘다(2026-08-08 운영에서 실제로 한 명이 그 상태였다). 커서는 성공할 때마다 반드시
앞으로 가므로 같은 커서가 두 번 필요해지지 않고, 재시도는 같은 커서라 그대로 멱등이다.
...
- 제외한 사실을 알리는 system 블록이나 대체 문장은 만들지 않는다.
- 기존 checkpoint에 남은 위기 원문도 같은 전환 근거가 있을 때 문장 단위로 제거하고, 과거 버전이 만든
  중립 대체 문구도 읽는 시점에 제거한다. 제거 뒤 내용이 없으면 `[지난 이야기]` 블록 자체를 보내지 않는다.
...
| 09:00 | 아침 일기 푸시. **현재 꺼져 있다**(`morning_push_enabled=False`). 새로 공개된 전날 미독 개인/운영자 일기만 대상, OPERATIONS의 활성화 절차 참고 |
