...
def _llm_timeout(
    ...
    return min(settings.llm_timeout_s, max(remaining, floor))
...
async def _run_one(
    ...
    *,
    ...
    async def _guarded() -> Any:
        ...
        async with sem:
            return await _execute(tool, call, ctx, session_factory)
...
async def run_turn(
    ...
    *,
    ...
    deadline = now() + config.turn_deadline_s
    ...
    results = apply_result_budget(results, config.tool_result_budget_tokens)
    ...
    return AgentTurn(
        text=step2.text or step1.text or "",
        calls=calls,
