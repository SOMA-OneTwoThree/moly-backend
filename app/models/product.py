...
class Product(Base):
    ...
    # none_as_null: None 대입이 jsonb 'null'로 저장되면 obj_ck CHECK 위반.
    ...
    # none_as_null: None 대입이 jsonb 'null'이 되면 hay_pack_ck의 `assets IS NULL`이 false가 됨.
    assets: Mapped[dict | None] = mapped_column(JSONB(none_as_null=True), nullable=True)
    # hay_pack 전용
