...
- 모든 유저는 유저·캐피 메시지와 **같은 트랜잭션**(커밋 전)에 turn 배정 + 추출 잡을 건다.
...
_REAL_RECORD = chat_service._record_memory_source
...
def _ctx() -> SimpleNamespace:
    return SimpleNamespace(anchor_message_id=0)
...
def _patch_repo(monkeypatch, calls: list, *, alloc_error: Exception | None = None,
    ...
    async def _alloc(session, *, user_id, representative_message_id, message_ids, committed_at):
        ...
        return memory_repo.SourceTurn(
            watermark=7, memory_generation=3, message_ids=tuple(message_ids),
        )
...
async def _post(session, monkeypatch, *, greeting_id=None, key="idem-w8"):
    ...
    monkeypatch.setattr(chat_service, "_record_memory_source", _REAL_RECORD)
...
# 1. 전 유저 단일 경로.
# ─────────────────────────────────────────────────────────────
async def test_user_without_chat_context_records_source(monkeypatch):
    calls: list = []
    ...
    assert [call[0] for call in calls] == ["alloc", "enqueue"]


# ─────────────────────────────────────────────────────────────
# 2. normalized — 메시지와 같은 트랜잭션.
# ─────────────────────────────────────────────────────────────
async def test_allocates_turn_and_enqueues_before_commit(monkeypatch):
    calls: list = []
    _patch_repo(monkeypatch, calls)
    session = _Session(get_map={"ChatContext": _ctx()})
...
async def test_committed_greeting_joins_the_same_turn(monkeypatch):
    ...
    session = _Session(
        get_map={"ChatContext": _ctx(), "Greeting": greeting}
    )
...
async def test_real_repo_path_writes_turn_and_job_in_the_phase2_transaction(monkeypatch):
    session = _RepoSession(get_map={"ChatContext": _ctx()})
