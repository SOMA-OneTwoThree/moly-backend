...
서버 문구·버튼 위치/크기/색 A→B 변경은 같은 dev TestFlight 25번에서 사용자 확인 완료다.
현재 확장은 `shape_v1` 시각 요소와 디자인 없는 `action_region_v1`이다. 서버가 이미지·텍스트·도형을 배치하고 `content_ids`로 클릭 영역에 연결한다. 새 표현은 새 capability를 지원하는 앱에서 검수하며, 25번의 확인 결과로 새 표현까지 검증했다고 간주하지 않는다.
