...
2026-09-13 사용자 요청 반영: 단순 재조회·시간대 변경은 같은 현지 날짜의 결과를 유지하지만, 생일·성별 수정으로 profile revision이 바뀌면 API result_invalidated=true로 오늘 결과를 무효화하고 다음 reveal에서 점수·카드·문구를 다시 계산한다. 기존 광고권한과 최초 이용일은 유지한다. 생일 A→B→A는 같은 날짜·모드·버전에서 A의 결과를 재현한다. 날짜를 변경해 다른 날 결과를 만든 뒤 되돌리면 같은 입력/같은 버전에서 결정적으로 재생성한다. 최신 한 건 구조이므로 과거 문구 버전까지 영구 복원한다는 보장은 하지 않는다. 결과 보존과 cross-user동일성은 반드시 버전과 발급시 생일을 포함한 조건부 계약이다. 삭제는 명시적 결과 reset이며 삭제한 생일을 별도 보관해 복원하지 않는다.
...
공개응답 schema3/점수숫자/문구배열형태 유지. 최초전용 화면필수아님. 생일·성별 수정 후 당일 결과가 이전 revision이면 result_invalidated=true를 반환하므로 클라는 기존 재조회 경로를 사용한다. 변경사항을 다음날 반영한다는 이전 안내는 적용하지 않는다. 운세정보삭제시 생일·결과는삭제하지만 첫이용여부중복방지용 날짜는계정탈퇴전까지 유지하므로 삭제안내와 개인정보정책 대조도 필요. 이 작업에서는 클라파일을 수정하지 않았다.
