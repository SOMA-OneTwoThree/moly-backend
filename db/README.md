...
제약·트리거·규칙·정책 추가와 권한 확대, 기본값 없는 필수 컬럼·UNIQUE 인덱스 추가는 거부한다.
FK 등 제약을 집행하는 내부 트리거도 참조하는 테이블·참조받는 테이블 양쪽의 활성 상태를 검사한다.
제약 정의와 validated 표시가 같아도 내부 트리거가 비활성화되어 있으면 실패한다.
