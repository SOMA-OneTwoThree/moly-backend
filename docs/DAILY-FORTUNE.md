...
> 기준일: 2026-09-08
>
> 문서 상태: 하루 총평 전면 재설계·세 언어 재집필·서버 및 개발 DB 검증 완료. PR·푸시·배포 미실행
>
...
새 총평 순열에는 `fortune-selection.v2`, 그대로인 분야 순열에는 `fortune-selection.v1`을 사용한다. 사용자 UUID·생년월일·
...
사용자가 승인한 10개 한국어 예시의 운세 말투와 하루 해석을 기준으로, 세 언어의 총평을 밑바닥부터 다시 작성했다.
서버 문구 버전은 `fortune-copy.v2-day-overview.1`, 선택 상태 버전은 `fortune-selection.v2`다.
점수 계산과 공개 응답 스키마 3은 유지한다. 분야별 800묶음은 이번 총평 재집필 대상이 아니며,
본문 전체의 문장 끝 마침표만 제거한다. 클라이언트 ARB·화면 연결과 실제 배포는 후속 작업이다.
...
- 한국어 총평에서 `~겠어`를 사용하지 않는다. 승인 예시처럼 친근한 운세 판정과 권유로 이어간다.
...
- 부록 B는 분야별 기존 40경로의 20묶음 전문이다. 총평 재집필과 범위를 구분한다.
...
| 검사 | 결과 |
| --- | --- |
| 새 총평 수량 | 한국어·영어·일본어 각 200묶음 / 1,200표현, 총 600묶음 / 3,600표현 |
| 원고와 서버 자산 | 최종 30개 점수별 원고와 실행 JSON의 모든 묶음 일치 |
| 문구 검사 | 총평 문장 끝 마침표·한국어 `~겠어`·제목 및 완성 해석 복제 0; 1,200표현 전체 정확 중복도 언어별 0 |
| 서버 회귀 | `uv run pytest -q --ignore=tests/integration` — 2,171개 통과 |
| 서비스/개발 DB 통합 | `MOLY_FORTUNE_TEST_ENV=dev uv run pytest -q tests/integration/test_fortune_variants.py` — 6개 통과 |
...
영어의 딱딱한 표현, 일본어 직역 결합을 지적해 수정했다. 각 언어 담당은 원고 전체의 의미·말투를 자기검수했다.
짧은 일상 표현이 겹친다는 이유로 의미를 바꾸거나 검사 통과용 수식어를 붙이지 않았다.

별도 에이전트가 새 선택·카탈로그 코드와 버전 전환·배포 계약을 읽어 검토했고, 진행을 막는 결함을 발견하지 못했다.
...
v1→v2 전환 시 분야별 순서 보존을 확인했다. 임시 테스트 데이터는 종료 시 삭제하고 잔여가 없음을 검사했다.
HTTP·광고 SDK·실제 구독 판정·천체 계산 정확성의 통합 검증으로 확대 해석하지 않는다.

이 기록은 에이전트 집필·교차검수 및 서버 테스트 결과다. 외부 사람 원어민의 전수 검수나 실제 기기 표시 검증을
수행했다는 뜻은 아니다. 이번 브랜치는 PR·푸시·배포하지 않았다.

### 4.8 현지 표현 참고 자료

앞선 2026-09-08 검수에서 확인한 장르 참고 자료다. 이번 재집필은 사용자가 승인한 10개 예시를 우선한다.
...
```json
{
  ...
  "versions": {
    ...
    "copy": "fortune-copy.v2-day-overview.1"
  }
}
```
...
- 코드와 세 언어 JSON·manifest를 함께 배포한다. 카탈로그는 `fortune-copy.v2-day-overview.1`, 선택 상태는 `fortune-selection.v2`다.
- DB 테이블·컬럼·제약·인덱스·RLS 변경은 없다. 운영 마이그레이션 SQL이나 일괄 데이터 재작성은 필요하지 않다.
- 이미 생성된 유효 당일 snapshot은 옛 문구와 말투·마침표까지 보존한다. 다음 날짜 또는 기존 freshness 조건에 따른 재계산에서 새 총평을 적용한다.
...
- **바로 이전 v1 선택 코드로 무작정 롤백하지 않는다.** 구 reader는 유효한 새 snapshot의 공개 형식을 읽을 수 있지만 구 writer는 다음 재계산 때 v2 상태를 거부한다. 이 버전 경계에서는 구 writer와 새 writer의 동시 가동을 피한다.
...
`fortuneResultGreeting`과 `fortuneSample*`의 문구는 프리뷰용이다. 서비스 결과는 4.6의 API 필드로 교체한다. 총평 프리뷰는 세 언어 모두 `overall.d70.general/v01`로 맞춰두었으며, 한 샘플 분야 본문을 모든 탭에 재사용하지 않는다.
...
| `fortuneSampleDetailBody` | 프리뷰 → API 결과 | 처음 움직일 때의 부담보다 해볼 수 있다는 마음이 커지는 흐름이야<br>완벽한 때를 기다리기보다 지금 가능한 시도가 기회를 열어줘<br>작게 시작해도 오늘은 그다음이 보일 수 있어 | The sense that you can try may grow stronger than the pressure of that first move<br>An attempt you can make now opens more room than waiting for perfect timing<br>Even a small beginning can help you see what comes next | 動き出す重さより、やってみようという気持ちが大きくなりそう<br>完璧なときを待つより、今できる挑戦が機会につながる流れ<br>小さく始めても、今日はその先が見えてくるかも |
