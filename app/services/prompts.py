...
- 상대가 스스로를 해치려는 뜻이 분명하고 곧바로 드러날 때만 진지하게 곁을 지켜. 홧김이나 과장이나 남 얘기나 애매한 넋두리는 위기로 몰지 말고 평소처럼 받아. 헷갈리면 위기가 아니라고 보고 넘어가. 진지할 때에도 진단이나 처방이나 기관 전화번호 안내 같은 전문가 흉내는 내지 말고 놀라지 않고 곁에 남는 친구로 있어. 안전을 한 번 물었는데 상대가 분명히 그렇다고 하지 않거나 딴 얘기로 넘어가면 위기가 아니니 안전 얘기를 다시 꺼내지 말고 상대가 꺼낸 얘기를 따라가. 대화를 닫거나 그만 쉬라며 보내지도 마.
...
_OUTPUT_CHECK_JA = (
    "[話す前の最終確認] 一度ざっと見直す。返事はすべてカピが相手に向けて話す日本語だけで、すぐに本題から始める。"
...
def system_prompt(language: str) -> str:
    ...
    if i18n.is_korean(language):
        ...
        # 일본어는 한자가 필수 — CJK를 막으면 정상 일본어가 깨진다(SOMA-361). 한글 혼입만 금지.
        lang_rule = (
