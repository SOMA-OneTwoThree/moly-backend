...
버튼의 실제 터치 영역은 최소 48px로 확장된다. 본문 frame이 이 영역과 겹치면 앱이 카드 전체를 숨긴다. 문장 위치와 클릭 영역을 함께 검증한다. 운세·상점 본문은 짧은 선 아래와 버튼 배경 위 사이에 중앙 정렬한다. 운세는 날짜·질문 두 줄 전체를 하나의 묶음으로 계산한다. 대화 주제는 배경 이미지의 두 긴 선 사이에 본문을 중앙 정렬하고, 제목은 위쪽 선의 오른쪽 위에 두며 별도 짧은 구분선은 쓰지 않는다. 버튼 그림과 최소 클릭 영역은 서로 다를 수 있다.
