...
async def run_flow(c, ext, db, uid, token):
    ...
    check("출석 +20 → 200", r.status_code == 200, str(r.status_code))
    bal = await db.fetchval("select hay_balance from profiles where id=$1", uidU)
    check("잔액 20 반영", bal == 20, f"balance={bal}")
    ...
    check("GET /wallet balance=20", r.status_code == 200 and r.json().get("balance") == 20)
    ...
    check("루틴 2개완료 보상 +20 → 200", r.status_code == 200, str(r.status_code))
    bal = await db.fetchval("select hay_balance from profiles where id=$1", uidU)
    check("잔액 40 반영(출석20+루틴20)", bal == 40, f"balance={bal}")
    r = await c.post("/charging-station/routine-reward")
