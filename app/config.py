...
class Settings(BaseSettings):
    ...
    # 기억 추출 단위 — 턴 하나가 아니라 **대화 덩어리**로 묶는다.
    #
    # 턴마다 뽑으면 한 사건이 여러 턴에 걸칠 때 조각난다(실측: 유저가 캐피 모습을 보고
    # "귀여워"라고 한 턴에서 무엇을 귀여워했는지가 빠졌다). 게다가 같은 주제가 이어지면
    # 비슷한 후보가 반복 생성돼 중복 판정이 7.7%였다.
    #
    # 실측(2026-08-08, 최근 7일): 유저 발화의 86%가 직전 발화로부터 3분 안에 이어진다.
    # 대화는 한자리에서 여러 턴을 주고받고 몇 시간 쉬는 모양이다. 그래서 180초를 고른다 —
    # 3분 안쪽이면 같은 자리의 대화로 보고 한 덩어리로 묶는다.
    #
    # **0으로 두면 예전처럼 턴마다 즉시 처리한다.** app_config 없이 env로 되돌릴 수 있게 남긴다.
    # 지연은 사용자에게 안 보인다 — 최근 대화는 원문 그대로 프롬프트에 들어가므로 회상이
    # 아직 없어도 캐피가 방금 한 얘기를 안다.
    memory_chunk_idle_s: float = 180.0
    # 한 잡이 먹는 최대 턴 수. 오래 안 오던 사용자가 돌아오면 수백 턴이 한 잡에 몰려
    # 프롬프트가 모델 상한을 넘고, 실패하면 그 전부를 다시 해야 한다. 남은 구간은 다음 잡이
    # 이어 먹는다(`enqueue_next_ingest`가 커서 뒤를 계속 물어온다).
    memory_chunk_max_turns: int = 20
    ...
    # AdMob 보상 계약을 대조한다. 공개 식별자인 iOS/Android placement를 혼합 배포용 기본값으로
    # 고정하고 infra에서도 명시한다. 명시적으로 빈 값을 주면 모든 건초 SSV를 fail-closed 거절한다.
    hay_ad_unit_ids: str = "3343480648,3086065971"
