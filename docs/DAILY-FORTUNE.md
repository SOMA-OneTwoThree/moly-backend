...
> 이전 기록(2026-09-13): [당시 원고 기준·서비스 참고·검증](fortune-content/fortune-editorial-reset-2026-09-13.md). 한국어·영어·일본어 각 780묶음과 첫 방문 6세트에 대한 당시 구현 기록이다.
>
> 기존 개편 설계: [긍정 경험·점성술/타로 유지 설계](fortune-content/positive-experience-plan.md). 점수·점성술·추첨·DB는 이번 한국어 재집필에서 변경하지 않는다.

> 기준일: 2026-09-15
>
> 문서 상태: 세 언어 원고·서버 자산 연결·정적 검증 완료. 개발 반영 대상이며 운영·모바일 배포는 별도
>
...
| `docs/fortune-content/ko-rewrite/` | 승인 한국어 정본·카드 근거·별도 검수 | 이번 현지화에서는 원고를 고정 |
| `docs/fortune-content/localization/` | 영어·일본어 정본·작성자와 별도 독해 검수·PR 준비 | 원고 변경 후 해당 번들 재독·정적 검사·실행 자산 재생성 |
| `app/resources/fortune/` | 실행용 세 언어 문구·계산 규칙·manifest | 문구는 정본 검수 후 `build_fortune_localized_copy.py`로 생성 |
| `docs/fortune-content/briefs.json` | 이전 버전 집필·현지화 기준 | 역사적 기록. 현재 기준은 ko-rewrite와 localization 사용 |
| `docs/fortune-content/review.json` | 이전 버전의 독해·의미 검수 기록 | 현재 원고는 ko-rewrite/review와 localization/review 참조 |
...
- [검수 기준·적용 계획](#4-문구와-표시-기준--2026-09-15)
...
| 오늘의 흐름 | 하루의 운 판단과 설명·행동을 담는 2단락 |
| 행동 | 오늘 해볼 것 1개, 오늘 조심할 것 1개 |
| 분야 | 애정·금전·학업/직장·건강 점수와 분야별 1~3단락 |
| 행운색 | 사전 정의된 12색 중 1개 |
...
- 유효한 당일 snapshot은 그대로 반환한다. 동일한 프로필값 재저장이나 같은 현지 날짜의 시간대 변경만으로 재발급하지 않는다. 실제 프로필 변경으로 revision이 달라지면 위 재계산 규칙을 따른다. 런타임 LLM이나 개별 문장 조합은 없다.
- 처음 공개한 결과와 한국어·영어·일본어 문구를 한 행에 저장해 그날 동안 그대로 반환한다.
...
총평만 읽어도 그날의 운을 이해할 수 있어야 한다. 재물은 돈이 들어오거나 나가는 판단을 먼저 밝히고, 건강은 몸의 불편과 부상 주의를 다룬다. 좋은 운·나쁜 운을 직접 말하되 카드의 정역 의미와 관계 조건을 유지한다. 카드 그림을 억지로 좁은 소재에 연결하거나 점수에 맞춰 문장을 조절하지 않는다. 상세 기준은 [한국어 집필 기준](fortune-content/ko-rewrite/CRITERIA.md)과 [현지화 기준](fortune-content/localization/README.md)을 따른다.

### 4.1 해석 근거와 품질 검수

`ko-rewrite/*.briefs.json`이 분야별 카드 키워드·운 판단·작성 조건·출처를 담는다. `tarot-editorial.v2.json`에는 이를 바탕으로 한 카드·방향·분야·중심 의미·세 언어 실행 문구 hash를 생성한다. 건강 진단이나 수익 보장을 번역 과정에서 덧붙이지 않는다. 카드명·정역·내부 메타데이터는 사용자 본문에 넣지 않는다.

한국어는 분야별 작성자와 root의 전량 독해를 마친 정본이다. 영어·일본어는 각 작성자가 전량 자체 재독하고, 별도 검수자가 목표 언어 전문을 먼저 읽은 뒤 한국어 운 판단·조건과 비교한다. root가 초기 원고·조합·수정 사항을 검토하고 최종 기록을 통합한다. 실제 독해 담당은 키별 `reviewer`에 남긴다.

검수 기록은 한국어 `ko-rewrite/review.json`, 새 현지화 `localization/review.json`이다. 자연스러움·가독성·분야 적합성·의미 일치·확신의 정도를 각각 판단한다. 작성자와 별도 검수의 현행 hash가 모두 일치하고 미해결 지적이 없어야 실행 자산을 생성한다. 해시 일치는 읽은 원고의 식별이며 언어 품질 자체의 증거가 아니다. 에이전트 검수를 인간 원어민 감수로 표현하지 않는다.

세 언어 대조 전문은 부록 A/B의 문서를 정본에서 생성한다. 기존 `review.json`, `*editorial-review.md`의 과거 통과 기록을 새 원고에 재사용하지 않는다.

### 4.2 실제 서비스 참고 범위

이번 현지화에서 실제로 읽은 현지 서비스 본문과 채택 범위는 `localization/en/author-review.json`, `localization/ja/author-review.json`에 기록한다. 아래 표는 2026-09-11 작업 당시의 참고 이력이다. 2026-09-13 재집필에서 실제로 확인한 서비스 본문과 접근 한계는 [새 참고 기록](fortune-content/fortune-editorial-reset-2026-09-13.md#실제-서비스-참고-기록)에 구분해 기록한다. 아래는 당시 공개 결과 본문을 읽고 정리한 편집 관찰이다. 서비스 문장을 차용하거나 해당 서비스가 품질을 인증했다는 뜻이 아니다. 과거 날짜의 결과도 문체 표본으로만 사용한다.
...
공개 API `schema_version: 3`과 필드명·광고 권한을 유지한다. 새 단락과 이전 snapshot을 모두 읽도록 공개 검증은 `flow` 2~3개, `text` 1~3개를 허용하고, 새 카탈로그는 위 분야별 단락 수를 엄격하게 검증한다. 현재 모바일은 배열 전체를 파싱하지만 한 문단으로 붙인다. 빈 줄 보존과 건강 제목(`건강 / Health / 健康`)을 위한 [모바일 표시 패치](fortune-content/localization/mobile-paragraph-display.patch)를 준비했고 모바일 작업 폴더에는 적용하지 않았다. 패치 적용 시 이전 snapshot의 구간도 각각 문단으로 표시된다. 운세 대화 등은 기존 공개 운세와 fingerprint 계약을 사용한다.
...
### 5.3 이전 v6 공개 응답 예시 — 2026-09-13

아래는 이전 코드·문구의 기록이다. 현재 v8 원고나 이번 작업의 API 실행 결과가 아니다. 새 전문은 부록 A/B의 대조표를 사용한다.

2002-12-13생, 2026-09-13, Asia/Seoul의 일반 이용 결과다. 실제 사용자 데이터나 배포된 API 응답이 아니라, 당시 작업 브랜치의 계산·추첨·공개 변환 코드로 생성했다. 합성 사용자 UUID는 `00000000-0000-4000-8000-000000000213`이며 활성화된 신규 경로에서는 추첨이나 점수의 시드에 들어가지 않는다. 같은 생일·날짜·이용 모드·알고리즘 및 문구 버전이면 사람 간에도 같은 결과를 생성한다. 성별과 표시 이름은 계산에 사용하지 않는다.
...
### 현재 2026-09-15 현지화와 연결 준비

이번 diff는 정본·세 언어 실행 문구·검수·카탈로그 단락 검증·공개 배열 개수와 회귀 검사 준비를 포함한다. DB 모델·DDL·점수·점성술·추첨·광고 권한·인프라 설정은 변경하지 않는다. 이번 변경으로 추가할 DB 마이그레이션이나 환경변수는 없다. 이전 기능의 DDL 이력을 이번 배포의 새 작업으로 다시 실행하지 않는다.

첫 방문 6세트의 ID와 카드·순서를 유지하고 해당 카드의 새 원고만 동기화한다. 유효한 당일 저장본은 유지하며 새 생성부터 새 문구를 사용한다. 서버 코드와 생성 자산·manifest는 함께 반영한다. 공개 계약 번들까지 갱신하되 모바일의 문단 표시 패치는 별도 적용 대상이다.

현재 실행 검증 범위와 CI·개발 배포 검증 절차는 [현지화 작업 기록](fortune-content/localization/README.md)에 구분한다. 개인 AWS·SSH·로컬 앱·Docker·로컬 DB·로컬 pytest는 사용하지 않는다. 개발 API의 기존 health 응답은 미배포 코드를 검증한 결과가 아니다. 후속 승인에 따라 dev PR·개발 배포 전후 실제 API·격리 계정 검증을 진행하고 결과를 PR에 기록한다. 운영은 변경하지 않는다.

### 이전 2026-09-13 작업과 배포 범위

당시 작업은 긍정 경험 로직과 당시 재집필 원고를 한 개발 브랜치에 모은 상태였다. 문구 버전은 `fortune-copy.v6-fortune.1`이고, 첫 방문 자산을 포함해 manifest가 관리하는 파일은 9개다. 아래 2026-09-11 및 그 이전 기록의 30점 하한, UUID 기반 추첨, DB 변경 없음이라는 설명은 당시 버전에만 해당한다.
