...
이미지 버튼의 응답 예시는 [composed_feed.json](../tests/fixtures/banners/composed_feed.json)을 참고한다. 현재 배너 파일은 운세·기능 업데이트(감정 기록)·상점·대화 주제·음악 추천 순서로 배경, 제목, 본문과 `shape_v1` + `text_v1` + `action_region_v1` 버튼을 정의한다. 운세 날짜는 `date` 요소의 `{day}`로 요청 시간대의 오늘을 표시하고, 질문은 `message` 요소로 분리해 굵기를 각각 조절한다. 버튼은 각각 기존 운세 화면, 감정 기록 팝업, 상점 화면, 주제 준비를 거친 대화 화면, 음악 선택 화면으로 이동한다. 음악 배너는 이미지에 그려진 컨트롤 위에 `음악 선택` 문구를 두며 하단 클릭 영역 전체가 음악 선택 화면을 연다. 이미지 속 재생·이전·다음 등은 독립적인 재생 컨트롤이 아니다. 정확한 필수 필드는 [서버 스키마](../app/schemas/banners.py)를 따른다.
