...
같은 앱 세션에서 이미 본 미완료 질문은 일반 대화 버튼으로 다시 들어가도 같은 준비 참조로 서버 상태를 확인해 복구한다. 질문 표시·복구만으로 LLM 호출이나 주제 소비는 없다. 만료·superseded는 입력을 보존하고 기존 재선택/일반 대화 안내로 처리한다. 완료·명시적 일반 대화 선택·로그아웃은 보관한 참조를 제거한다. 프로세스 종료 후 일반 진입의 자동 질문 복구는 제공하지 않으며 배너 재진입으로 서버 상태를 복구한다.
